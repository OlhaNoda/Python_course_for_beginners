...
def sum_of_digits_recursive(digit_string: str) -> int:
    if not isinstance(digit_string, str):
        raise TypeError(f'The function {sum_of_digits_recursive.__name__} works only with digit_string: str')
    if digit_string != '':
        if not digit_string.isdigit():
            raise TypeError(f'The function {sum_of_digits_recursive.__name__} works only with digit string')
    if digit_string == '':
        return 0
